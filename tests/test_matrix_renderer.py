...
MAIN = '\nint main(){\n for(int n=0;n<29;n++){drawIdleFrame(n);uint8_t full[104];std::memcpy(full,matrix.frame,104);drawIdleFrame(n,2);for(int i=0;i<104;i++){assert(matrix.frame[i]<=2);assert((full[i]==0)==(matrix.frame[i]==0));}}\n for(int state=0;state<=8;state++){\n  requestedStatus=state;requestedProfile=11;drawnStatus=-1;requestedAttract=0;refreshMatrix();uint8_t reference[104];std::memcpy(reference,matrix.frame,104);\n  if(state!=PG_GESTURES_IDLE){for(int mode=1;mode<=2;mode++){drawnStatus=-1;set_powerglove_attract(mode,3);refreshMatrix();assert(std::memcmp(reference,matrix.frame,104)==0);}}\n }\n requestedStatus=PG_GESTURES_IDLE;drawnStatus=-1;set_powerglove_attract(2,3);refreshMatrix();for(int i=0;i<104;i++)assert(matrix.frame[i]==(i==91||i==93||i==95?1:0));\n set_powerglove_attract(2,0);refreshMatrix();assert(matrix.frame[91]==1&&matrix.frame[93]==0&&matrix.frame[95]==0);\n}\n'
