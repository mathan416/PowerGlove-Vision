...
p = argparse.ArgumentParser(description="Install PowerGlove Vision on retropie. Run as your normal login user.")
...
try:
    ...
    with tempfile.TemporaryDirectory(prefix="powerglove-download-") as temporary:
        ...
        for line in (directory / "SHA256SUMS").read_text().splitlines():
            ...
            sums[name] = digest
        for name in ("install-package.py", "PowerGlove-Vision-RetroPie.zip"):
        ...
        cmd = ["sudo", "python3", str(directory / "install-package.py"), "retropie",
               "--archive", str(directory / "PowerGlove-Vision-RetroPie.zip"), "--version", tag]
        if a.peer:
            cmd += ["--peer", a.peer]
