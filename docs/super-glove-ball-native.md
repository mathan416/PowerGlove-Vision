...
| Cabinet field mapping and stabilization | Implemented and live-tested; synchronized physical latency remains open | Continuous X/Y uses fresh, geometry-validated MediaPipe palm observations, capture timestamps, per-player asymmetric reach, and an edge clamp. Latest is the default and is direct during continuous tracking; bounded uses one two-dimensional calibrated-noise response with no prediction or overshoot. Brief loss holds only X/Y for up to 120 ms and releases actions immediately. Latest accepts strongly aligned forward recovery at once, while one contradictory or unusually distant non-forward recovery waits for the next fresh result. The optical-flow experiment is archived. FCEUmm D-pad thresholds are unchanged. |
