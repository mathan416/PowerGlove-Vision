...
# PowerGlove Vision — Setup Cheat Sheet

Use this reference with your own installation. Replace `UNO-Q-NAME.local`
with your UNO Q hostname and `RETROPIE-NAME.local` with your RetroPie hostname.
Commands run from your downloaded project directory unless stated otherwise.
Never include private pairing tokens in documentation.
...
| Network hostname | `UNO-Q-NAME.local` |
| IPv4 address | Check your router or the board network settings |
| Board | Arduino UNO Q |
| App Lab app | PowerGlove Vision |
| Camera | UVC-compatible USB camera, configured as `auto` |
| Receiver hostname | `RETROPIE-NAME.local` |
...
| Purpose | URL |
| --- | --- |
| Live debug dashboard | <http://UNO-Q-NAME.local:8088/debug> |
| Offline gesture lessons | <http://UNO-Q-NAME.local:8088/learn> |
| Friendly connection setup | <http://UNO-Q-NAME.local:8088/setup> |
| Secure pairing setup | <https://UNO-Q-NAME.local:8443/setup> |
| Root (redirects to dashboard) | <http://UNO-Q-NAME.local:8088/> |
| Machine-readable status | <http://UNO-Q-NAME.local:8088/status> |
| Camera stream | <http://UNO-Q-NAME.local:8088/stream> |
| GitHub repository | <https://github.com/mathan416/PowerGlove-Vision> |

If `.local` discovery is unavailable, replace `UNO-Q-NAME.local` with the current
UNO Q IP address shown by your router or board network settings.
...
| Item | Value |
| --- | --- |
| Console hostname | `RETROPIE-NAME.local` |
...
| UNO Q shutdown watcher | `powerglove-system-shutdown.path` (verify it is enabled and active) |
...
Install the shutdown helper before using the shutdown control. **Stop controller**
leaves the UNO Q running; **Shutdown** requests a Linux halt. The tested board
restarts afterward, so remaining stopped is not guaranteed.
...
Configure SSH access to your UNO Q first. From your local project directory,
deploy application updates and restart PowerGlove Vision with:
...
Your UVC-compatible USB camera must be connected to the UNO Q through an externally powered
USB-C hub. Connect to App Lab over Wi-Fi while the UNO Q is acting as the USB
host. A camera connected to your computer is not visible inside the UNO Q app.

If the UNO Q lists only `qcom-venus-encoder` and `qcom-venus-decoder`, your camera
is not visible to Linux. Check the powered hub, reconnect your camera, wait several
