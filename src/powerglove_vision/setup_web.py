...
function renderPairing(){
 ...
 $('pair-prerequisite').textContent=!savedConfig?'Load your saved settings before pairing.':!savedConfig.connection_configured?'Enter your console address above and save Connection and startup before pairing.':dirtySettings()?'You have unsaved changes. Save them above before pairing.':'';
...
function renderCameraProfile(state){
 ...
 const view=$('camera-profile-view'),frame=$('camera-profile-frame'),measuring=running&&state.phase==='measuring';
 view.hidden=!measuring;
 if(measuring&&!frame.getAttribute('src'))frame.src=frame.dataset.src+'?t='+Date.now();
 if(!measuring&&frame.getAttribute('src'))frame.removeAttribute('src');
 const instruction=String(state.instruction||'').toLowerCase(),cue=instruction.includes('opposite corners')?'sweep':instruction.includes('edge')?'edge':'centre';
 view.dataset.cue=cue;$('camera-profile-cue').textContent=cue==='sweep'?'Sweep smoothly between opposite corners':cue==='edge'?'Touch an edge, then return to centre':'Keep your whole hand near the centre';
