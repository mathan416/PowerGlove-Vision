...
<button id=player-reuse type=button hidden>Reuse my saved center</button><details><summary>Players and hand-setup backups</summary><p>New players start with the current sensitivity and their own lesson progress. Each player keeps a saved center. Switching players pauses controls; set a fresh center or explicitly reuse their saved center if the camera and playing position are unchanged.</p>
...
<p>Backups include your name, personal adjustments, complete sensitivity values, software identity, and this player’s saved calibration. Restore lets you choose saved sensitivity values or the installed defaults. Pairing credentials, Wi-Fi settings, and lesson progress are excluded. Reuse calibration only with the same camera position and playing position; otherwise set a fresh center.</p>
...
el('player-reuse').onclick=()=>{if(confirm('Are your camera position and playing position unchanged from this player’s saved center? Reuse requires the same physical setup.'))command('reuse_calibration',{confirmed:true})};
