...
if(el('player-export'))el('player-export').onclick=async()=>{try{const s=await api('export'),raw=String(s.backup?.name||context?.players?.find(p=>p.id===context.active)?.name||'player'),slug=raw.normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,40)||'player';const blob=new Blob([JSON.stringify(s.backup,null,2)+'\n'],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=slug+'-powerglove-hand-setup.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(e){notice(e.message)}};
if(el('player-import'))el('player-import').onchange=async()=>{const file=el('player-import').files?.[0];if(!file)return;try{if(file.size>8192)throw Error('Choose a hand-setup JSON file smaller than 8 KB.');const backup=JSON.parse(await file.text());if(!backup||typeof backup!=='object'||backup.format!=='powerglove-hand-setup'||![2,3].includes(backup.version))throw Error('Choose a version-2 or version-3 hand-setup backup. Version-1 backups are not supported.');importBackup=backup;importIdentity={player:context?.active,generation:context?.generation};const complete=backup.format==='powerglove-hand-setup'&&[2,3].includes(backup.version),hasCalibration=complete&&backup.calibration!=null;el('restore-description').textContent='Restore '+(typeof backup.name==='string'?backup.name:'this backup')+' to the current player: replace sensitivity, center-box size'+(complete?' and player name':'')+'. '+(hasCalibration?'A saved calibration is available.':'No saved calibration is included; set a fresh center.');el('restore-description').textContent+=' Software: '+(backup.source?.version||'not recorded')+' · '+(backup.source?.commit||'commit not recorded')+'.';el('restore-effective').checked=false;el('restore-effective').disabled=!backup.effective_thresholds;el('restore-reuse').checked=false;el('restore-reuse').disabled=!hasCalibration;el('restore-review').hidden=false}catch(e){notice(e.message)}finally{el('player-import').value=''}};
