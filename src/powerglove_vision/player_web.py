...
if(el('player-import'))el('player-import').onchange=async()=>{const file=el('player-import').files?.[0];if(!file)return;try{if(file.size>8192)throw Error('Choose a hand-setup JSON file smaller than 8 KB.');const backup=JSON.parse(await file.text()),current=backup?.format==='virtualglove-hand-setup'&&backup?.version===4,legacy=backup?.format==='powerglove-hand-setup'&&[2,3].includes(backup?.version);if(!backup||typeof backup!=='object'||(!current&&!legacy))throw Error('Choose a VirtualGlove hand-setup backup or a legacy version-2/version-3 backup. Version-1 backups are not supported.');importBackup=backup;importIdentity={player:context?.active,generation:context?.generation};const complete=current||legacy,hasCalibration=complete&&backup.calibration!=null;el('restore-description').textContent='Restore '+(typeof backup.name==='string'?backup.name:'this backup')+' to the current player: replace sensitivity, center-box size'+(complete?' and player name':'')+'. '+(hasCalibration?'A saved calibration is available.':'No saved calibration is included; set a fresh center.');el('restore-description').textContent+=' Software: '+(backup.source?.version||'not recorded')+' · '+(backup.source?.commit||'commit not recorded')+'.';el('restore-effective').checked=false;el('restore-effective').disabled=!backup.effective_thresholds;el('restore-reuse').checked=false;el('restore-reuse').disabled=!hasCalibration;el('restore-review').hidden=false}catch(e){notice(e.message)}finally{el('player-import').value=''}};
