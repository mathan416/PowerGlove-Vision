...
<p id=finger-feedback class=label></p><div class=lesson-result id=lesson-result>Waiting for your hand…</div><div class=lesson-progress id=lesson-progress></div>
...
const lessons=[
 ...
 {title:'Curl your index finger',image:'finger-curl.png',cue:'Keep your palm visible and curl your index finger toward it.',ok:s=>(s.fingers?.index||0)>=2,result:'Index curl recognized.'},
 ...
 {title:'Push toward the camera',image:'push-toward-camera.png',instant:true,cue:'Move your open hand closer to the camera in one deliberate push.',ok:s=>s.buttons?.glove_zap||(s.events||[]).includes('glove_zap'),result:'Push recognized — training complete!'}
];
...
function draw(s={}){const lesson=lessons[index];$('lesson-image').src='/help-assets/gestures/actions/'+lesson.image;$('lesson-image').alt=lesson.title;$('finger-feedback').textContent=lesson.menu?'Finger curl (0 straight, 3 closed): '+Object.entries(s.fingers||{}).map(([name,value])=>`${name} ${value}`).join(' · '):'';$('lesson-number').textContent=`Lesson ${index+1} of ${lessons.length}`;$('lesson-title').textContent=lesson.title;$('lesson-cue').textContent=lesson.cue;$('lesson-progress').innerHTML=lessons.map((_,i)=>`<i class="${completed.has(i)?'done':i===index?'current':''}"></i>`).join('');$('previous').disabled=index===0;$('next').textContent=index===lessons.length-1?'Start again':'Skip lesson';$('tracking').textContent=s.detected?'Hand found':'No hand';$('recognized').textContent=action(s);$('confidence').textContent=`${Math.round((s.confidence||0)*100)}%`;}
