...
const performance=s=>{const p=s.performance||{},inference=p.inference_ms||{},age=p.capture_age_ms||{},sample=p.sample_age_ms||{},transition=p.controller_transition_age_ms||{};return [`Model: ${s.tracker_backend_label||s.tracker_backend||'—'}`,`${s.motion_tracking?"Movement updates":"Recognition"}: ${s.inference_hz??'—'} Hz`,`${s.motion_tracking?"Movement processing p50 / p95":"Inference p50 / p95"}: ${inference.p50??'—'} / ${inference.p95??'—'} ms`,`Camera read → send p50 / p95: ${sample.p50??'—'} / ${sample.p95??'—'} ms`,`Changed control → send p50 / p95: ${transition.p50??'—'} / ${transition.p95??'—'} ms`,`${s.motion_tracking?"Frame waiting before processing p50 / p95":"Frame waiting before inference p50 / p95"}: ${age.p50??'—'} / ${age.p95??'—'} ms`,`Superseded camera frames: ${s.capture_skipped_total??0}`,`Preview encode: ${s.preview_encode_ms??'—'} ms`,...(s.motion_tracking?[`Gesture recognition (latest): ${s.recognition_inference_ms??'—'} ms`,`Gesture source age: ${s.gesture_age_ms??'—'} ms`,`Palm flow: ${s.motion_valid?'tracking':'waiting for reliable hand recognition'}`]:[])].map(x=>`<div class=label style="margin:5px 0">${x}</div>`).join('')};
