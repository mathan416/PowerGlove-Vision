 ...
 try{
  ...
  if(!matches(config)){put('saved','attention','The selected console or startup profile has unsaved changes. Select Save connection, then check again.');}
   ...
   try{
    ...
    if(s.worker_running===true&&s.game_session_active===true&&['super_glove_ball','bad_street_brawler',...'abcdefghi'.split('').map(x=>'program_'+x)].includes(profile)&&typeof core==='string'&&core&&core!=='unknown'&&['native','joystick'].includes(mode)){
